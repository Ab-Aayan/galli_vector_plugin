...
/// * the collection of [GalliMarker]s added to this map
...
///
/// GalliMarker tap events can be received by adding callbacks to [onGalliMarkerTapped].
...
class GalliMapController extends ChangeNotifier {
  GalliMapController({
      ...
      for (var type in annotationOrder.toSet()) {
        ...
        switch (type) {
          ...
          case AnnotationType.symbol:
            symbolManager = GalliMarkerManager(this,
                onTap: onGalliMarkerTapped,
  ...
  GalliMarkerManager? symbolManager;
  ...
  final ArgumentCallbacks<GalliMarker> onGalliMarkerTapped =
      ArgumentCallbacks<GalliMarker>();
  ...
  /// been notified.
  Future<GalliMarker> addGalliMarker(GalliMarkerOptions options,
      [Map? data]) async {
    final effectiveOptions =
        GalliMarkerOptions.defaultOptions.copyWith(options);
    final symbol = GalliMarker(getRandomString(), effectiveOptions, data);
  ...
  /// The returned [Future] completes with the added symbol once listeners have
  /// been notified.
  Future<List<GalliMarker>> addGalliMarkers(List<GalliMarkerOptions> options,
      [List<Map>? data]) async {
    final symbols = [
      for (var i = 0; i < options.length; i++)
        GalliMarker(getRandomString(),
            GalliMarkerOptions.defaultOptions.copyWith(options[i]), data?[i])
    ];
  ...
  Future<void> updateGalliMarker(
      GalliMarker symbol, GalliMarkerOptions changes) async {
    await symbolManager!
  ...
  Future<LatLng> getGalliMarkerLatLng(GalliMarker symbol) async {
  ...
  ///
  /// The returned [Future] completes once listeners have been notified.
  Future<void> removeGalliMarker(GalliMarker symbol) async {
  ...
  ///
  /// The returned [Future] completes once listeners have been notified.
  Future<void> removeGalliMarkers(Iterable<GalliMarker> symbols) async {
  ...
  ///
  /// The returned [Future] completes once listeners have been notified.
  Future<void> clearGalliMarkers() async {
    symbolManager!.clear();
    notifyListeners();
  }
  ...
  ///
  /// This allows you to add an image to the currently displayed style once, and from there on refer to it e.g. in the [GalliMarker.iconImage] anytime you add a [GalliMarker] later on.
  ...
  Future<void> setGalliMarkerIconAllowOverlap(bool enable) async {
    await symbolManager?.setIconAllowOverlap(enable);
  }

  /// For more information on what this does, see https://docs.mapbox.com/help/troubleshooting/optimize-map-label-placement/#label-collision
  Future<void> setGalliMarkerIconIgnorePlacement(bool enable) async {
    await symbolManager?.setIconIgnorePlacement(enable);
  }

  /// For more information on what this does, see https://docs.mapbox.com/help/troubleshooting/optimize-map-label-placement/#label-collision
  Future<void> setGalliMarkerTextAllowOverlap(bool enable) async {
    await symbolManager?.setTextAllowOverlap(enable);
  }

  /// For more information on what this does, see https://docs.mapbox.com/help/troubleshooting/optimize-map-label-placement/#label-collision
  Future<void> setGalliMarkerTextIgnorePlacement(bool enable) async {
  ...
  ///shows 360 lines when lines are not visible and hides them if already visible
  show360Lines() {
    if (three60lines) {
      three60lines = false;
      return _mapboxGlPlatform.setLayerVisibility("pano-layer", false);
    } else {
      three60lines = true;
      return _mapboxGlPlatform.setLayerVisibility("pano-layer", true);
    }
  }

  Future<String?> reverGeoCoding(LatLng latlng) async {
    Point<double> point =
        await _mapboxGlPlatform.toScreenLocation(latlng).then((value) {
      return Point(value.x.toDouble(), value.y.toDouble());
    });
    List<dynamic> layerIds = await _mapboxGlPlatform.getLayerIds();
    List<dynamic> data = await _mapboxGlPlatform.queryRenderedFeatures(
        point, layerIds.map((e) => e.toString()).toList(), null);
    String? locationString = data.isNotEmpty
        ? (data.first["properties"]["name"] == null
                ? ""
                : "${data.first["properties"]["name"]}") +
            (data.first["properties"]["properties"] != null &&
                    data.first["properties"]["name"] != null
                ? " ,"
                : "") +
            (data.first["properties"]["properties"] == null
                ? ""
                : "${data.first["properties"]["properties"]}")
        : null;
    return locationString;
  }

  // getImage(LatLng latlng) async {
  //   Point<double> point =
  //       await _mapboxGlPlatform.toScreenLocation(latlng).then((value) {
  //     return Point(value.x.toDouble(), value.y.toDouble());
  //   });
  //   List<dynamic> data = await _mapboxGlPlatform.queryRenderedFeatures(
  //       point, ["pano-layer"], null);
  // String? locationString = data.isNotEmpty
  //     ? (data.first["properties"]["name"] == null
  //             ? ""
  //             : "${data.first["properties"]["name"]}") +
